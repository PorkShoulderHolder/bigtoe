...
The benefit of this formulation, is that we can now differentiate this ratio with respect to each $K(\tau)$ at each position $\tau$ within the kernel domain, using function composition and back-propagation, and estimate the optimal value of the kernel at each $tau$. We can also plug in the kernel regression module within any subsequent differentiable operators and perform multiple tasks such as classification, reconstruction or beyond via gradient descent and back-propagation.

This formulation easily extends beyond single time series. Assuming that we now have $D$ time series with 
