...
function setup_network(labix, countX)
	...
	kernel_matrix_init = torch.mm(cov_row,normal_kernel:view(1,normal_kernel:size(1)))
	-- gnuplot.figure(9)
	...
	conv_layer_top = nn.SpatialConvolutionMM(1,1,halfkwidth*2+1,covmatrix:size(1))
...
function regress(labix)
	...
	for i= 1,x1:size(2) do
		if x[{{labix},{i},{}}]:gt(0):sum() > 2 then
			local input = pad(x[{{},{i},{}}]:clone():view(1,1,100,109):cuda(), halfkwidth)
			...
			local target = x[{{labix},{i},{}}]:cuda()			

			for t =1, x1:size(3) do
				if inputnnx[1][1][labix][t+halfkwidth] ~= 0 then
		...
		for labixx = 1, input:size(3) do
			...
			for tix = halfkwidth+1, input:size(4)-halfkwidth do			
				if nnxlab[tix] == 1 and tix ~= t then
					...
					if tix+jump > halfkwidth and tix+jump < input:size(4)-halfkwidth then
	...
	for epoch = 1,maxEpoch do		
		...
		for ox = 1, x1:size(3)*x1:size(2) - 1 do
			...
			if x1[labix][i][t] ~= 0 and x1[{{labix},{i},{}}]:gt(0):sum() > 2 then
				...
				local input = pad(x[{{},{i},{}}]:clone():view(1,1,100,109):cuda(), halfkwidth)
				local target = input[1][1][labix][padded_t]
				input[{{1},{1},{labix},{padded_t}}]:fill(0)
		...
		gnuplot.figure(1)
		gnuplot.imagesc(conv_layer_top.weight:float():view(100,normal_kernel:size(1)):clone(),'color')
		gnuplot.figure(2)
		gnuplot.plot(conv_layer_top.weight:float():view(100,normal_kernel:size(1)):clone():abs():mean(1):squeeze(),'-')
		gnuplot.figure(3)
		gnuplot.plot(conv_layer_top.weight:float():view(100,normal_kernel:size(1)):clone():mean(2):squeeze(),'-')
